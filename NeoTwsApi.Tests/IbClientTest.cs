...
#region HistoricalData
    ...
    {
        Contract contract = MsftContract;
        DateTime start    = DateTime.Parse("2023/12/11");       // ��һ
        ...
        Contract contract = EurContract;
            ...
            {// 20��
                var ret2 = await client.ReqHistoricalDataAsync2(contract, DateTime.Parse("2004/02/25", null, DateTimeStyles.AssumeUniversal), DateTime.Parse("2024/02/25", null, DateTimeStyles.AssumeUniversal),
                                                               ETimeFrameTws.W1, EDataType.MIDPOINT);
                ret2.FirstOrDefault().Time.Should().Be("20050307");       // eurusd ���20050307
                ret2.LastOrDefault().Time.Should().Be("20240219");
...
#region Streaming Data
    ...
    {
        //Contract            contract     = EurContract;  // Ĭ��ֻ��eur֧��tickbytickdata�� ����ֻ��midpoint��bidask
        //Contract            contract     = AaplContract;  // Ĭ��ֻ��eur֧��tickbytickdata�� ����ֻ��midpoint��bidask
        //Contract            contract     = TmfContract;  // Ĭ��ֻ��eur֧��tickbytickdata�� ����ֻ��midpoint��bidask
        Contract contract = XauusdContract_CMDTY; // xau��֧��tickbytick
        //Contract            contract     = BacContract;  // Ĭ��ֻ��eur֧��tickbytickdata�� ����ֻ��midpoint��bidask
...
#region Orders
    ...
    public async Task PlaceOrderAsync_Test()
    {
        ...
        Order order = new Order
        {
            ...
            //TotalQuantity = 1,
            CashQty = 38930,
            //TotalQuantity = 2000.01m,   ��tws�ͻ����ϣ�eur��С֧��0.01���ӣ�Ҳ���Ա�ִ�У�����ͨ��api�޷�ִ�У�����TwsErrorCodes.OrderNotSupportFractionalQuantity
 ...
 public async Task PlaceStopOrderAsync_Test()
 {
    ...
    public async Task ModifyOrderAsync_Test()
    {
        ...
        r.Should().NotBeNull();
        r.Order.TotalQuantity.Should().Be(40000);       // �޸ĳ�40000
 ...
 {// �޸����ŵ�����Ҫͬʱ�޸��������ӵ�
     ...
     // �޸���order ��������ͬʱ�޸�2���ӵ�����
     {
      ...
      //�޸�takeprofit��ֻ���޸��ӵ���takeprofit
      {
          ...
          modifyTakeprofitOrder.OrderId = ret[1].OrderId; // ��Ӧtakeprofit����

          var r2 = await client.PlaceOrderAsync(EurContract, modifyTakeprofitOrder);

          r2.Should().NotBeNull();
          r2.Order.LmtPrice.Should().Be(1.35); // �޸ĳ�1.35
      }
      // �޸�stoploss��ֻ���޸��ӵ���stoploss
      {
          ...
          modifyStoplossOrder.OrderId = ret[2].OrderId; // ��Ӧstoploss����

          var r3 = await client.PlaceOrderAsync(EurContract, modifyStoplossOrder);

          r3.Should().NotBeNull();
          r3.Order.AuxPrice.Should().Be(0.95); // �޸ĳ�0.95
      }
